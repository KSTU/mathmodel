...
for ii=1:str2num(arg_list{2})
	...
	fprintf(file_id,"\\item По экспериментальным данным (из справочника теплофизических свойств чистых веществ) получить описание давления паров чистого компонента от температуры. Для %s использовать уравнение %s, для %s использовать уравнение %s. На основе полученных уравнений и закона Рауля, для смеси %s построить $T-x,y$ диаграмму равновесия пар-жидкость при давлении %6.1f мм.рт.ст. . Сравнить результаты полученные по модели с экспериментальными данными и сделать вывод о применимости модели.\n\n",file.subs1{nid},equa(neq1,:),file.subs2{nid},equa(neq2,:),file.mix{nid},cur_p)
	fprintf(file_id,"\\item Построить $p-x,y$ диаграмму при температуре %6.1f $^\\circ$C. Сравнить результаты полученные по модели с экспериментальными данными и сделать вывод о применимости модели. ",cur_t)
