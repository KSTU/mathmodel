...
function out=z4(id)
	...
	fprintf(id,"\\item Машина трогается с места и за %4.1f минуты разгоняется с постоянным ускорением до %3.1f ${\\frac{км}{ч}}$, далее машина %4.0f минут едет с постоянной скоростью, после чего начинает торможение с постоянным ускорением и через %3.0f минут останавливается. Определить путь пройденный машиной. ",t1,v1,t2,t3);
endfunction
...
function out=z5(id)
	...
	"Реализовать алгоритм определения корня уравнения методом деления отрезка пополам. "
