...
Использовать модель идеального вытеснения для обоих потоков.\
 ...
 теплопроводность материала стенки $\\lambda_%s=%5.0f~\\frac{Вт}{м \\cdot К}$.\
  Горячий теплоноситель (обозначен индексом <<h>>) направляется во внутреннюю трубу и\
	 имеет следующие параметры: температура $T_%s=%4.0f~^\\circ\\mathrm{C}$, теплоемкость\
	  $c_{p%s}= %5.1f~\\frac{Дж}{кг \\cdot ^\\circ\\mathrm{C}}$, теплопроводность \
		$\\lambda_%s= %4.3f~\\frac{Вт}{м \\cdot ^\\circ\\mathrm{C}}$, плотность \
		$\\rho_%s=%5.0f \\frac{кг}{м^3}$, коэффициент вязкости $\\mu_%s=%5.3f мПа \
		\\cdot с $, коэффициент термического расширения $\\beta_%s=%7.6f ^\\circ\\mathrm{C}^{-1}$,\
		 расход $G_%s= %5.3f \\frac{кг}{ч}$. Холодный теплоноситель (обозначен индексом <<c>>) \
		 направляется в межтрубное пространство и имеет следующие параметры: температура $T_%s= %4.0f\
		 ~^\\circ\\mathrm{C}$, теплоемкость $c_{p%s}=%5.0f \\frac{Дж}{кг \\cdot ^\\circ\\mathrm{C}}$,\
			 теплопроводность $\\lambda_%s=%4.3f \\frac{Вт}{м \\cdot ^\\circ\\mathrm{C}}$, плотность \
			 $\\rho_%s= %5.0f~\\frac{кг}{м^3}$, коэффициент вязкости $\\mu_%s=%5.3f~мПа \\cdot с $, \
			 расход $G_%s=%5.2f~\\frac{кг}{ч}$. \n\n" ,L,D2,D1,wall,del,wall,la_st,hot_s,T2,hot_s,cp2,hot_s,la2,hot_s,ro2,hot_s,mu2,hot_s,bet,hot_s,g2,cold_s,T1,cold_s,cp1,cold_s,la1,cold_s, ro1,cold_s,mu1,cold_s,g1)
...
fprintf(file_id,"\\textsc{\\textbf{Лабораторная работа <<Моделирование теплообменника типа труба в трубе>>}}\n\n")
