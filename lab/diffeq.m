...
function out=fun_id(x,y,z,fid)
	switch (fid)
		case 1
			out.n="\\sqrt{|x-y+z|}";
			out.res=sqrt(abs(x-y+z));
		case 2
			out.n="\\sqrt{|x+y+z|}";
			out.res=sqrt(abs(x+y+z));
		case 3
			out.n="\\dfrac{x+y}{10+\\sin(z)}";
			out.res=(x+y)/(10+sin(z));
		case 4
			out.n="sin(x+y+z)";
			out.res=sin(x+y+z);
		case 5
			...
			out.res=sin(x-y+z);
		case 6
			out.n="\\dfrac{x^{0.8}}{5+sin(y)}";
			out.res=x^0.8/(5+sin(y));
		case 7
			out.n="2x-3y";
			out.res=2*x-3*y;
		case 8
		...
		case 11
			...
			out.res=x-2*y;
		case 12
			out.n="\\dfrac{y}{10+\\cos(z)}";
			out.res=y/(10+cos(z));
		case 13
			out.n="\\dfrac{3+x+y}{10+x^2+z^2}";
			out.res=(3+x+y)/(10+x^2+z^2);
		case 14
			out.n="x^{1/3}";
			out.res=x^(1/3);
	endswitch
...
function out=z4(id)
	...
	if(nz==5)
		...
		t1=t2+10+rand()*100;
		K=300+rand()*300;
		fprintf(id," Тело массой %2.1f кг и площадью поверхности %2.1f м$^2$ охлаждается в помещении с постоянной температурой воздуха %2.1f $^\\circ\\mathrm{C}$. Теплоемкость материала %4.1f $\\frac{\\text{Дж}}{\\text{кг}\ \\text{град}}$. Записать уравнение описывающее изменение температуры тела во времени (при условии высокой теплопроводности материала тела). Определить за какое время температура тела опустится с %3.1f $^\\circ\\mathrm{C}$ до %3.1f $^\\circ\\mathrm{C}$ при постоянном коэффициенте теплопередачи равным %5.0f $\\frac{\\text{Вт}}{\\text{м}^2 \\text{град.}}$ \n\n",mass,S,to,cp,t1,t2,K);
	endif
	if(nz==6)
		la1=20+rand()*100;
		la2=0.5+rand()*0.5;
		...
		T1=500+rand()*300;
		q=200+rand()*300;
		fprintf(id," Записать дифференциальное уравнение распределения температуры вдоль стенки, материал которой имеет следующую зависимость теплопроводности от темепратуры: $\\lambda=%3.1f+%3.1f\\text{T}$. Построить распределение темпарутры по толщине стенки толщиной %3.1f см при температуре стенки с одной стороны равной %4.0f K и тепловом потоке %3.1f Вт/м. Определить температуру с другой стороны стенки.\n\n ",la1,la2,tol,T1,q)
	endif
endfunction
...
function out=z5(id)
	...
	if(nz==2)
		...
		fprintf(id," В прямоугольный бак сечением %3.1f см x %3.1f и высотой %3.1f см поступает %3.1f л в секунду. На дне имеется отверстие площадью %3.1f $\\text{см}^2$. За какое время наполнится бак? Записать  дифференциальное уравнение изменения уровня жидкости (или объема воды в баке) от времени, построить график. Сравнить результат с временем заполнения этого бака без отверстия.\n\n ",a,b,h,ras,S)
	endif
	if(nz==3)
		r=15+rand()*10;
		h=20+rand()*40;
	...
	if(nz==4)
		...
		K=400+rand()*400;
		te=500+rand()*300;
		fprintf(id," При комнатной температуре 20$^\\circ\\mathrm{C}$ в печь поместили заготовку массой %3.1f кг и поверхностью %3.1f $\\text{м}^2$, после включения температура воздуха в печи равномерно увеличивалась на %2.1f градуса в минуту. Плотность материала заготовки --- %4.1f $\\text{кг}/\\text{м}^\\mathrm{3}$, теплоемкость --- %4.1f $\\frac{\\text{Дж}}{\\text{кг} \\cdot град}$. Коэффициент теплопередачи между воздухом и металлом равен %5.0f $\\frac{Вт}{м^2 град.}$. Определить через какое время металл нагреется до температуры %5.0f градусов.",m,S,dt,ro,cp,K,te)
	endif
	if(nz==5)
		D=5+rand()*50;
		V=50+rand()*100;
		...
		Tv=20+rand()*20;
		Ts=Tv+20+rand()*60;
		fprintf(id," Сфера диаметром %3.1f см, имеющая температуру %3.1f $^\\circ\\mathrm{C}$, для охлаждения была опущена в сосуд объемом %4.0f л, заполненный водой, имеющей температуру %3.1f $^\\circ\\mathrm{C}$ . Плотность материала сферы --- %4.1f \\text{кг}/\\text{м}$^\\mathrm{3}$, теплоемкость --- %4.1f $\\frac{\\text{Дж}}{\\text{кг}\\cdot \text{град}}$, плотность воды равна 1000 $\\text{кг}/\\text{м}^\\mathrm{3}$, теплоемкость воды --- 4200 $\\frac{\\text{Дж}}{\\text{кг}\\cdot \\text{град}}$, коэффициент теплоотдачи равен %5.0f $\\frac{\\text{Вт}}{\\text{м}^2 \\text{град.}}$. Записать систему дифференциальных уравнений изменения температуры воды и сферы при условии отсутствия теплообмена с окружающей средой. Построить график зависимости температуры воды и сферы от времени. Определить время, при котором разница между температурой воды и сферы равна 5 $^\\circ\\mathrm{C}$. \n\n",D,Ts,V,Tv,ros,cps,K);
	endif
endfunction
...
fprintf(id, "С самолета выброшен предмет сечением %3.1f $\\text{м}^2$ и массой %3.1f кг с начальной нулевой вертикальной скоростью. Численно решить дифференциальное уравнение изменения скорости от времени, при условии, что сила сопротивления воздуха пропорциональна квадрату скорости и площади сечения предмета $f = k S \\upsilon^2$, где k = %5.4f --- коэффициент пропорциональности. Построить график изменения скорости от времени. Определить скорость предмета через %3.0f секунд.", S, m, k, t)
endif
...
fprintf(id, "Тело брошено под углом %3.0f градусов к поверхности земли c начальной скоростью %3.1f м/с. Не учитывая сопротивления воздуха решить дифференциальное уравнение движения. Построить траекторию движения тела. Определить через какое время тело будет на высоте %3.0f м", u, v, h)
endif
...
fprintf(id, "Тело брошено под углом %3.0f градусов к поверхности земли c начальной скоростью %3.1f м/с. Не учитывая сопротивления воздуха решить дифференциальное уравнение движения. Построить траекторию движения тела. Определить через какое время тело упадет на землю.", u, v)
endif
...
fprintf(id, "Тело брошено под углом %3.0f градусов к поверхности земли c начальной скоростью %3.1f м/с. Не учитывая сопротивления воздуха решить дифференциальное уравнение движения. Построить траекторию движения тела. Определить какое расстояние пролетит тело.", u, v)
endif
...
if(length(arg_list)<2)
	%if((arg_list(1)==null)||(arg_list(2)==null))
		fprintf("Error. Usege: octave diffeq.m group_number variant_number\n");
		break
	%endif
endif
